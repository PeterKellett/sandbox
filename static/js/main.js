$(document).ready(function () {
    ...
    $("#submit1").click(function () {
        ...
        console.log(vanHeight)
        for (let i = Math.round(vanWidth / 10) * 10; i >= 0; i -= 20) {
            tetaRad = Math.asin(vanHeight / i);
            tetaDeg = tetaRad / (Math.PI / 180);
            grad = Math.tan(tetaRad);
            casterHeight = wcLength * Math.sin(tetaRad);
            mechanicalSpace = vanWidth - i;
            $("#ramp-data").append("<tr><td>" + i + " mm <strong>/</strong> " + mechanicalSpace + " mm" + "</td><td>" + tetaDeg.toFixed(2) + " deg" + "</td><td>" + (grad * 100).toFixed(2) + " %" + "</td><td>" + casterHeight.toFixed(2) + " mm" + "</td></tr>");
            if (Number.isNaN(tetaDeg)) {
                break;
            }
        }
